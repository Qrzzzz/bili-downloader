...
- 列表项使用分隔线与单层详情；文件按需读取，日志使用独立 ContentDialog。
- 原生 CommandBar 处理窄窗口操作溢出，保留队列和账号授权边界。
- 版本 3.3 / 程序集 3.3.0.0 / IPC v2；本地候选验证见 [3.3 验证记录](./docs/validation/v3.3.md)。

## v3.2 界面与队列角标

- 在 3.1 基础上统一原生界面细节，任务导航角标反映完整队列中的等待及执行任务，筛选和页面切换不改变计数。
- 版本 3.2 / 程序集 3.2.0.0 / IPC v2，依赖和后端执行协议保持不变；验证范围见 [3.2 验证记录](./docs/validation/v3.2.md)。
- 用户已授权完整发布流程并要求不重新下载 Release 产物；采用工作流验证与远端 API digest 核对。
