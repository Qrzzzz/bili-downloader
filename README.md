...
**3.3**：任务页改为紧凑列表与单层详情，支持直接打开文件和独立日志窗口。见 [3.3 发布说明](./docs/releases/v3.3.md)。
...
<p>
  ...
  <a href="./docs/releases/v3.3.md">v3.3 发布说明</a> ·
  <a href="#主要功能">主要功能</a> ·
...
**v3.3** 任务页减少卡片嵌套，按任务状态展示操作；保留队列角标、下载模式与画质偏好。见 [3.3 发布说明](./docs/releases/v3.3.md) 和 [验证记录](./docs/validation/v3.3.md)。

Windows x64 正式版本请从 [GitHub Releases](https://github.com/Qrzzzz/bili-downloader/releases/latest) 获取。3.3 的资产名称为：

* 完整应用包：`BiliDownloader.v3.3.win-x64.zip`
* 软件物料清单：`BiliDownloader.v3.3.sbom.json`
* 校验文件：`SHA256SUMS`

完整解压对应版本的 ZIP 后运行其中的 `BiliDownloader.v3.3.exe`，保留相邻的后端 EXE 和运行库。包中包含 .NET、Windows App SDK 和 Python 运行时，无需另装这些运行环境。下载前请自行准备合法来源的 `ffmpeg.exe`，并选择以下任一方式放置：
...
1. 运行完整解压目录中的 `BiliDownloader.v3.3.exe`。
...
7. 下载页主动选择的 MP4/MP3 模式和画质会自动保存；在“设置 → 下载偏好”可直接调整，或关闭自动记忆。关闭后，本次会话的选择不会覆盖保存的偏好。指定画质不可用时需重新选择，MP3 不会清除视频画质偏好；设置变化不影响已入队任务。
...
* 支持分享文本与 Markdown 链接提取，保留分 P 参数并去掉分享追踪参数
...
# 构建自包含目录与 ZIP 候选包
.\build.ps1 -Clean
.\tools\package_smoke.ps1 -Executable .\dist\BiliDownloader.v3.3.win-x64\BiliDownloader.v3.3.exe
```

当前源码构建输出为 `dist\BiliDownloader.v3.3.win-x64\` 与同名 ZIP；未提交构建会保留 dirty 标记。`build.ps1` 重新创建 `build\.venv`，使用哈希锁与 NuGet locked restore。`python -m app.main` 是兼容后端入口，UI 入口已迁移至 WinUI 工程。
