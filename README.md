...
<p>
  ...
  <a href="./docs/releases/v2.9.md">v2.9 发布说明</a> ·
  <a href="#主要功能">主要功能</a> ·
...
当前版本为 **v2.9**：修复多实例凭据换代竞态、扫码成功状态和迟到验证结果。见 [发布说明](./docs/releases/v2.9.md) 和 [验证记录及待人工项](./docs/validation/v2.9.md)。

Windows x64 正式版本请从 [GitHub Releases](https://github.com/Qrzzzz/bili-downloader/releases/latest) 获取。v2.9 资产为：

* 完整应用包：`BiliDownloader.v2.9.win-x64.zip`
* 软件物料清单：`BiliDownloader.v2.9.sbom.json`
* 校验文件：`SHA256SUMS`

完整解压对应版本的 ZIP 后运行其中的 `BiliDownloader.v2.9.exe`，保留相邻的后端 EXE 和运行库。包中包含 .NET、Windows App SDK 和 Python 运行时，无需另装这些运行环境。下载前请自行准备合法来源的 `ffmpeg.exe`，并选择以下任一方式放置：
...
1. 运行完整解压目录中的 `BiliDownloader.v2.9.exe`。
...
# 构建自包含目录与 ZIP 候选包
.\build.ps1 -Clean
.\tools\package_smoke.ps1 -Executable .\dist\BiliDownloader.v2.9.win-x64\BiliDownloader.v2.9.exe
```

当前源码构建输出为 `dist\BiliDownloader.v2.9.win-x64\` 与同名 ZIP；未提交构建会保留 dirty 标记。`build.ps1` 重新创建 `build\.venv`，使用哈希锁与 NuGet locked restore。`python -m app.main` 是兼容后端入口，UI 入口已迁移至 WinUI 工程。
