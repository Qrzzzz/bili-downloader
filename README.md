...
一个 Windows x64 单窗口桌面程序，用于解析并下载用户有权访问的 Bilibili 视频内容。当前源码与候选发布版本为 **1.3**；已公开版本以 [GitHub Releases](https://github.com/Qrzzzz/bili-downloader/releases) 为准。
...
v1.3 候选 Release 标题固定为 `Bili Downloader Lite v1.3`，并应只包含：

- `BiliDownloader.v1.3.exe`
- `BiliDownloader.v1.3.sbom.json`
- `SHA256SUMS`

版本只使用两级数字。源码 `1.3`、标签 `v1.3`、PE 元数据、文件名与 Release 标题必须一致。
...
`requirements.in`、`requirements-dev.in` 和 `requirements-sbom.in` 记录直接依赖；对应 `.txt` 是 Python 3.13/Windows x64 哈希锁。v1.3 的二维码渲染依赖为无传递依赖的 `segno==1.6.6`。
...
默认 onedir 输出为 `dist\BiliDownloader\BiliDownloader.v1.3.exe`，onefile 输出为 `dist\BiliDownloader.v1.3.exe`。`build.ps1` 每次删除并重建 `build\.venv`，不复用开发环境。
...
v1.2 的 DPAPI canonical session schema 保持可读。程序仍支持旧 `storage_state.json` / `cookies.txt` 的一次性安全迁移，并清理旧 `playwright-profile` / `login-cache` 残留；这些名称只存在兼容清理边界中。
...
程序只展示 yt-dlp 在当前账号、视频、地区、平台策略和支持能力下实际解析到的格式。FFmpeg 用于合并音视频流；v1.3 不捆绑 `ffmpeg.exe` 或 `ffprobe.exe`，用户需自行合规安装并加入 `PATH`。
