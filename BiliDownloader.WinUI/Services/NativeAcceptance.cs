...
public static class NativeAcceptance
{
    public static async Task CaptureAsync(Window window, FrameworkElement root, TitleBar title, NavigationView navigation, Frame frame)
    {
        ...
        if (Environment.GetCommandLineArgs().Contains("--ui-regression"))
        {
            ...
            var taskNavigation = navigation.MenuItems.OfType<NavigationViewItem>().First(i => (string)i.Tag == "tasks");
            // Exercise the actual shell bindings against the same isolated queue as the page.
            taskNavigation.DataContext = new { Tasks = fixtureQueue };
            root.RequestedTheme = ElementTheme.Light;
            ...
            navigation.SelectedItem = taskNavigation;
            await Task.Delay(300);
            page = frame.Content as Page ?? throw new InvalidOperationException("任务页未完成导航。");
            context = page.DataContext;
            page.DataContext = fixtureQueue;
            window.AppWindow.Resize(new Windows.Graphics.SizeInt32((int)(800 * root.XamlRoot.RasterizationScale), (int)(640 * root.XamlRoot.RasterizationScale)));
            navigation.IsPaneOpen = false;
            await Snapshot("tasks-compact-badge", true);
            Check(Visible(taskNavigation.InfoBadge), "native_badge_visible_in_compact_navigation");
            root.RequestedTheme = ElementTheme.Dark;
            window.AppWindow.Resize(new Windows.Graphics.SizeInt32((int)(640 * root.XamlRoot.RasterizationScale), (int)(480 * root.XamlRoot.RasterizationScale)));
            ...
            Check(taskNavigation.InfoBadge.Value == 3, "native_queue_badge_independent_of_visible_filter");
            ...
            taskNavigation.ClearValue(FrameworkElement.DataContextProperty);
