...
- 任务页使用分隔线列表与单层详情，去除卡片套折叠面板和空指标留白。
- 单文件任务直接打开，多文件在详情中逐项打开；保存位置只显示一次，失败分 P 保留原因。
- 操作随任务状态显示，窄窗口自动溢出到更多菜单；日志改为独立对话框并支持复制。
- 前后端、程序集及发行配置统一为 3.3；队列协议与下载执行机制保持不变。

## [3.2] - 2026-09-16

### 界面打磨与队列角标

- 任务导航使用原生数字角标，实时显示等待及执行中的任务数量；暂停队列仍保留等待数，完成后自动隐藏，筛选不影响计数。
- 统一页面标题语义、卡片圆角与间距，强化视频来源和设置分组；优化任务状态标签、空状态与完成记录清理入口。
- 保留原生主题、紧凑导航、键盘操作和现有队列协议；应用版本统一为 3.2。

## [3.1] - 2026-09-12

- 记住下载模式与偏好画质，支持设置默认偏好和关闭自动记忆；严格画质不可用时提示重新选择。

## [3.0] - 2026-09-12

### 任务化与并行下载

- 新增任务页面和持久队列，支持独立取消、失败重试、继续未完成项、队首调整、筛选及文件详情。
- 默认同时执行 2 个任务（可设置 1—3 个），下载时可继续解析；新增多链接批量解析及逐项配置。
- SQLite 事务保存任务与结果，提交幂等、实例归属和显式重启恢复；新输入不再使已入队任务的重试失效。
- 同名输出跨进程协调，任务独立暂存并在验证后原子发布；增加磁盘空间预算和 FFmpeg 处理互斥。
- 升级应用与程序集版本至 3.0 / 3.0.0.0，使用 IPC v2；保持旧配置、登录态及媒体校验边界。

## [2.12] - 2026-09-12

- 支持直接粘贴标题与链接、多行分享文本和 Markdown 链接，自动提取并校验视频 URL；保留分 P，去除分享追踪参数，重复链接去重，多个不同链接提示只保留一个。
- 下载页输入框支持多行与自动换行，并明确提示可粘贴分享文本。

## [2.11] - 2026-09-12

- #28：线程构造失败只返回一次请求错误；已接受操作启动失败时发送一次合法失败终态并释放 active，后续任务、取消和退出均能继续收尾。
- #29：合并或 MP3 转换期间已延迟的取消继续等待当前文件完成限时完整性验证；通过后保留成功状态及路径，再取消尚未开始的分 P。损坏、规格不符或超时的输出仍被拒绝，普通媒体验证仍可取消。

## [2.10] - 2026-09-08

- #24：复用和新输出均需通过限时、可取消的完整解码检查；MP3 音频流须为 192 kbps。损坏或规格不符的同名文件保留并改用新名称。
- #25：输出名称选择、下载选项准备和空间检查纳入逐项错误处理，保留此前成功项及路径，重试合并不丢失成功结果。
- #26：顺序下载时先验证当前项能否复用，仅为需下载的当前项检查剩余空间；保留估算值加 5% 和 16 MiB 的预留。
