...
def test_release_version_is_3_2_and_windows_compatible() -> None:
    app = importlib.import_module("app")
    version_tool = importlib.import_module("tools.write_version_info")

    assert app.__app_name__ == "Bili Downloader Lite"
    assert app.__version__ == "3.2"
    assert version_tool._numeric_version(app.__version__) == (3, 2, 0, 0)
    ...
    assert "StringStruct('FileVersion', '3.2')" in resource
    assert "StringStruct('ProductVersion', '3.2')" in resource
...
def test_artifact_audit_rejects_missing_xaml_resource_index() -> None:
    ...
    package = {name: None for name in (
        "BiliDownloader.v3.2.exe", "BiliDownloader.v3.2.dll", "Assets/AppIcon.ico",
        "BiliDownloader.Backend.exe", "Microsoft.ui.xaml.dll", "build-info.json",
        "backend-runtime/build-info.json",
    )}
    with pytest.raises(ValueError, match=r"Missing native package files:.*BiliDownloader.v3.2.pri"):
        audit._audit_contents(package, lambda _: b"", Path("unused.exe"), "3.2", None, False)
...
def test_workflows_pin_actions_and_keep_public_network_out_of_quality() -> None:
    ...
    assert "tags:\n      - v3.2" in release
    assert "RELEASE_TITLE: Bili Downloader Lite v3.2" in release
    assert "BiliDownloader.v3.2.win-x64.zip" in release
    assert 'docs/releases/v3.2.md' in release
...
def test_published_release_matches_verified_local_assets(
    ...
    commit = "a" * 40
    asset_names = ["BiliDownloader.v3.2.win-x64.zip", "BiliDownloader.v3.2.sbom.json", "SHA256SUMS"]
    assets = []
    ...
    release = {
        "tag_name": "v3.2",
        "name": "Bili Downloader Lite v3.2",
        "draft": False,
        "prerelease": False,
    ...
    def fake_gh_json(*args):
        if args == ("api", "repos/example/project/releases/tags/v3.2"):
            return release
        if args == ("api", "repos/example/project/git/ref/tags/v3.2"):
    ...
    monkeypatch.setattr(sys, "argv", [
        ...
        "--tag", "v3.2", "--expected-version", "3.2",
        "--expected-commit", commit, "--expected-title", "Bili Downloader Lite v3.2",
