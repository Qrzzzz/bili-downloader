...
def test_release_version_is_2_1_and_windows_compatible() -> None:
    app = importlib.import_module("app")
    version_tool = importlib.import_module("tools.write_version_info")

    assert app.__app_name__ == "Bili Downloader Lite"
    assert app.__version__ == "2.1"
    assert version_tool._numeric_version(app.__version__) == (2, 1, 0, 0)
    ...
    assert "StringStruct('OriginalFilename', 'BiliDownloader.v2.1.exe')" in resource
    assert "StringStruct('FileVersion', '2.1')" in resource
    assert "StringStruct('ProductVersion', '2.1')" in resource
...
def test_workflows_pin_actions_and_keep_public_network_out_of_quality() -> None:
    ...
    assert "BiliDownloader.v2.1.exe" in quality
    assert "--expected-version 2.1" in quality
    ...
    assert "tags:\n      - v2.1" in release
    assert "RELEASE_TITLE: Bili Downloader Lite v2.1" in release
    assert "BiliDownloader.v2.1.exe" in release
    assert 'docs/releases/v2.1.md' in release
