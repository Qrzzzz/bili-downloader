...
def test_protocol_rejects_ambiguous_and_unbounded_json():
    ...
    for line in (b'{"v":2,"v":2}', b'{"n":NaN}', b'[]', b'\xff', b'x' * 65537):
        with pytest.raises(ProtocolError):
            decode_request(line)
    valid = {"v": 2, "type": "request", "id": "r1", "method": "hello", "params": {}}
...
def backend():
    ...
    send("hello", {"protocol_version": 2, "frontend_version": "3.3"})
...
def test_handshake_and_duplicate_requests_fail_closed(backend):
    host, messages, send = backend
    assert not send("hello", {"protocol_version": 1, "frontend_version": "3.3"})["ok"]
...
'''
    requests = [dict(v=2, type='request', id='r1', method='hello', params={'protocol_version': 2, 'frontend_version': '3.3'}),
    ...
    assert output[0]['result']['backend_version'] == '3.3'
