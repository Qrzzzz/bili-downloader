...
def backend():
    ...
    send("hello", {"protocol_version": 1, "frontend_version": "2.9"})
...
def test_handshake_and_duplicate_requests_fail_closed(backend):
    ...
    assert not send("hello", {"protocol_version": 2, "frontend_version": "2.9"})["ok"]
...
'''
    requests = [dict(v=1, type='request', id='r1', method='hello', params={'protocol_version': 1, 'frontend_version': '2.9'}),
    ...
    assert output[0]['result']['backend_version'] == '2.9'
