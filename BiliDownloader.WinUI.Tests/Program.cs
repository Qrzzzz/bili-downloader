...
try
{
    ...
    await Test("real Qt-free backend: handshake, validation, settings, clean shutdown", async () =>
    {
        ...
        try
        {
            var hello = Protocol.Read<Hello>(await c.RequestAsync("hello", new { protocol_version = 1, frontend_version = "2.6" }));
            Check(hello.BackendVersion == "2.6" && hello.ProtocolVersion == 1, "Version negotiation failed");
